...
function[dout, dvalid, result] = utpfb_fir_generic_bbox_sim(varargin)
result = -1; dout = [];
...
[dvalid, temp, results(8)]           = utpar_get({args, 'dvalid'});
...
raw_dvalid      = {'dvalid', dvalid};
raw_inputs      = {raw_sync_in, raw_data_in, raw_dvalid};
...
raw_dvalid = utpar_get({raw_outputs, 'dvalid'});

[dout, en, dvalid, result] = utextract('mux', 2^n_inputs, 'data', raw_dout, 'sync', raw_synco, 'dvalid', raw_dvalid);
if result ~= 0, 
