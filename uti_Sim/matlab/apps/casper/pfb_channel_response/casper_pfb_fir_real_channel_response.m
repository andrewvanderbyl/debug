...
function[dout, result] = casper_pfb_fir_real_channel_response(varargin)
  warning off Simulink:Engine:OutputNotConnected
  ut_log_group = 'casper_pfb_fir_real_channel_response_debug';
  ...
  defaults = { ...
    ...
    'freq_step', 1/10, ...          % frequency step for each iteration in fraction of an fft_bin
    'span', 1, ...                  % frequency span over which to iterate
  ...
  pfb_fir_model_name    = 'pfb_fir_real_test';
  pfb_fir_block_name    = 'pfb_fir_real';
  pfb_fir_type          = 'default';
  pfb_fir_block_version = '1'; 
  ...
  [pfb_fir_model, result] = utpfb_fir_real_bbox_modelify( ...
...
%    din              = repmat([zeros(1,2^fft_stages*(n_taps-1)), 0.5.*ones(1,2^fft_stages)], 1, n_spectra);
    ...
    [pfb_fir_dout, result] = utpfb_fir_real_bbox_sim( pfb_fir_model{:}, ...
