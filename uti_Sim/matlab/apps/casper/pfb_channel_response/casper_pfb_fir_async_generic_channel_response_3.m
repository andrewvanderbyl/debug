...
function[dout, result] = casper_pfb_fir_async_generic_channel_response_3(varargin)
  %tic  
  ...
  defaults = { ...
    'n_spectra', 2^3, ...             % the amount of averaging to perform   (was 8)
    ...
    'offset', 100, ...                % offset of start frequency in fft bins
    'freq_step', 1/3, ...          % frequency step for each iteration in fraction of an fft_bin     1/15
  ...
  pfb_fir_model_name    = 'pfb_fir_generic_async_test_sim3';
  ...
  fft_model_name        = 'fft_async_test_sim3';
  ...
  %get response
  for freq_index = 5 + offset/fs:1:(offset+span)/fs-1,
    freq_index
    tic
...
%    din              = repmat([zeros(1,2^fft_stages*(n_taps-1)), 0.5.*ones(1,2^fft_stages)], 1, n_spectra);
    ...
    [pfb_fir_dout, pfb_fir_dvalid, result] = utpfb_fir_generic_bbox_sim( pfb_fir_model{:}, ...
        'name', pfb_fir_block_name, 'din', din', 'dvalid', dvalid, 'debug', 'off');     %MOD - dvalid
...
%        'name', pfb_fir_block_name, 'din', din', 'debug', 'off'); 
    ...
    pfb_fir_dout_test = reshape(pfb_fir_dout, 2^fft_stages, n_spectra);
